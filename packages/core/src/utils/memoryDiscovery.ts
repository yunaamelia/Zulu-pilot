...
async function getGeminiMdFilePathsInternal(
  ...
  const dirs = new Set<string>([...includeDirectoriesToReadGemini, currentWorkingDirectory]);
...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    try {
      ...
      if (debugMode) logger.debug(`Found readable global ${geminiMdFilename}: ${globalMemoryPath}`);
    } catch {
    ...
    if (dir && folderTrust) {
      ...
      if (debugMode) logger.debug(`Determined project root: ${projectRoot ?? 'None'}`);

      const upwardPaths: string[] = [];
      let currentDir = resolvedCwd;
      const ultimateStopDir = projectRoot ? path.dirname(projectRoot) : path.dirname(resolvedHome);
...
export async function loadGlobalMemory(debugMode: boolean = false): Promise<MemoryLoadResult> {
  ...
  const foundPaths = (await Promise.all(accessChecks)).filter((p): p is string => p !== null);
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No GEMINI.md files found in hierarchy of the workspace.');
  ...
  if (debugMode) logger.debug(`Combined instructions length: ${combinedInstructions.length}`);
