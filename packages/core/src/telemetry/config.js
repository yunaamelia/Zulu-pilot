...
export async function resolveTelemetrySettings(options) {
  ...
  const rawTarget = argv.telemetryTarget ?? env['GEMINI_TELEMETRY_TARGET'] ?? settings.target;
  ...
  const rawProtocol =
    argv.telemetryOtlpProtocol ?? env['GEMINI_TELEMETRY_OTLP_PROTOCOL'] ?? settings.otlpProtocol;
